...
/*
 * Timer2 巡线状态机业务约定：
 * LINE      普通单线巡线。只有一个传感器识别黑线时进入，动作交给 RunLinePid。
 * JUNCTION  十字路口。中线、左侧、右侧同时识别时进入；比赛规则要求直行，不转弯。
 * TURN      转向处理。多个传感器识别但不是十字时进入，覆盖普通弯、直角、T 字入口。
 * SEARCH    全白找线。五路传感器都没有黑线时进入，按上一次转向方向原地旋转。
 * LOCKOUT   十字冷却。离开十字后临时禁止再次识别十字，避免同一个路口区域重复触发。
 */
...
/*
 * 协作调试重点：
 * junction_state 是当前赛道元素处理状态，所有 Timer2 决策都围绕它展开。
 * last_turn_dir 是方向记忆，默认右转；TURN 和 SEARCH 都复用这个方向。
 * junction_tick 只记录压在十字上的持续周期，目前不做超时退出。
 * lockout_tick 记录 LOCKOUT 中连续看到单传感器的次数，达到 LOCKOUT_LINE_TICKS 后恢复 LINE。
 */
 ...
 * line_sum 左侧为负、右侧为正；line_error 取反后，线偏右时左轮加速、右轮减速，车向右修。
 * line_count 为 0 时不计算，避免全白或异常输入导致除零。
 */
...
void Timer2_ISR_Handler (void) interrupt TMR2_VECTOR		//进中断时已经清除标志
{
		...
		/*
		 * 基础业务判定：
		 * is_white    五路全白，优先进入 SEARCH。
		 * is_junction 中线 + 左侧 + 右侧同时有黑线，按十字直行处理。
		 * is_line     只有一个传感器有黑线，按普通 LINE 巡线处理。
		 * 其他多个传感器组合统一进入 TURN。
		 */
		...
		/*
		 * 状态转移优先级：
		 * 1. 全白直接 SEARCH，用 last_turn_dir 找线。
		 * 2. 已在 JUNCTION 时，只有离开十字组合才进入 LOCKOUT。
		 * 3. 已在 LOCKOUT 时，不重新触发十字；必须稳定看到单线才回 LINE。
		 * 4. 普通状态下，再按 junction / line / turn 判断。
		 * 下面只改状态，不直接输出 PWM；PWM 在末尾统一根据 junction_state 输出。
		 */
		if(is_white)
		{
		...
		else
		{
				...
				/* JUNCTION：十字区域内持续直行；一旦传感器不再满足十字组合，进入 LOCKOUT。 */
				...
				/* LOCKOUT：忽略新的十字触发；连续看到单传感器，说明车身已离开路口区域。 */
				...
				{
						/* TURN：多传感器但不是十字。只有单侧明确有线时更新方向；左右都有时沿用上次方向。 */
		 ...
		 * SEARCH / TURN 用原地旋转；JUNCTION 直行；LINE 和 LOCKOUT 走 PID。
		 */
